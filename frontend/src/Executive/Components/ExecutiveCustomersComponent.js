...
import FetchCustomerServices from '../Services/FetchCustomerServices'
const ExecutiveCustomersComponent = () => {
    const [CustomerToggle, setCustomerToggle] = useState(false)
    const { data, setdata, getallcustomers, currentpage, setcurrentpage, totalpages, customers } = FetchCustomerServices()
    ...
    return (
        <div>
                                    ...
                                    <div className="card-body">
                                        <ul className="nav nav-tabs nav-tabs-custom nav-success mb-3" role="tablist">
                                            <li className="nav-item">
                                                <a className="nav-link active" data-bs-toggle="tab" href="#nav-border-top-all" role="tab" aria-selected="true">Page-{currentpage}</a>
                                            </li>
                                        </ul>
                                                                            ...
                                                                            <td>
                                                                                <div className="dropdown">
                                                                                    ...
                                                                                    <ul className="dropdown-menu dropdown-menu-end">
                                                                                        <li>
                                                                                            <a onClick={() => { localStorage.setItem("Customerinfo", JSON.stringify({ id: customer._id })); navigate('/ExecutiveTransactionlist') }}> <button className="dropdown-item"><i className="las la-eye fs-18 align-middle me-2 text-muted" />
                                                                                                View</button></a>
                                                                                        </li>
                                                                                    </ul>
                                                                                </div>
            ...
            {CustomerToggle && <AddCustomerModalByExecutive getallcustomers={getallcustomers} setToggle={setCustomerToggle} />}
        </div>
    )
}
