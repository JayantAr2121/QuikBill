...
import AddShopkeeperService from '../Services/AddShopkeeperService'

const CreateAccountModal = (props) => {
    const { displayotpsection, otploading, loading, obj, shopkeepers, otp, AllState, setotp, set, getallcityandstate, getallshopkeepers, sendotp, submit } = AddShopkeeperService(props)
                            ...
                            {displayotpsection ? <form onSubmit={submit}>
                                ...
                                <div className="row">
                                    <div className="col-lg-12">
                                        ...
                                        <div className="row">
                                            {
                                                AllState && Object.keys(AllState).length !== 0 && <div className="col-6">
                                                    <div className="mb-4">
                                                        <label htmlFor="State" className="form-label">State</label>
                                                        <select className="form-select" onChange={set} name='state' aria-label="Default select example">
                                                            <option value="None">Select State</option>
                                                            {Object.keys(AllState)?.map((state, index) => <option key={index} value={state}>{state}</option>)}
                                                        </select>
                                                    </div>
                                                </div>
                                            }
                                            {
                                                AllState && Object.keys(AllState).length !== 0 && obj?.state && obj?.state !== "None" && <div className="col-6">
                                                    <div className="mb-4">
                                                        ...
                                                        <select className="form-select" onChange={set} name='city' aria-label="Default select example">
                                                            ...
                                                            {AllState[obj?.state]?.map((city, index) => <option key={index} value={city}>{city}</option>)}
                                                        </select>
                                                    </div>
                                                </div>
                                            }
